...
bool selfcompare(const string& s1, const string& s2,bool time){
   unsigned n2 = s2.size();
   unsigned n1 = s1.size();
   unsigned x=0;
   if(time==true)
...
CmdParser::listCmd(const string& str)
{
   ...
   if(s.size()==0){
      ...
      for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
         i++;
   	 istr[0]=(it->first)+((it->second)->getOptCmd());
         cout << setw(12) << left << istr[0];
 	 if(i==5){
    	    cout<<endl;
	    i=0;
	 }
      ...
      for(int i=0;i<str.size();i++)
         _readBuf[i]=str[i];
      cout << _readBuf;
      _readBufPtr = _readBufEnd = _readBuf + str.size();
   }   
   else{
      string cmd[11];
      for(it=_cmdMap.begin();it!=_cmdMap.end();it++){       
         cmd[cmdnum]=(it->first)+((it->second)->getOptCmd());
         cmdnum++;
      }
      for(int i=0;i<11;i++){
         if((selfcompare(cmd[i],s2,false))==true){
            if(s2.size()==cmd[i].size())
               flag=true;
            else if((selfcompare(cmd[i],s,false))==true){
               istr[num]=cmd[i];
               num++;
             }
         }
      ...
      if(flag==true){
         cout<<endl;
         getCmd(s2)->usage(cout);
         cout<<endl;
         string s3=_readBuf;
         resetBufAndPrintPrompt();
         for(int i=0;i<s3.size();i++)
            insertChar(s3[i]);
         moveBufPtr(_readBufPtr-s3.size()+str.size());
         flag=false;
      }
      else if(num==0)
         mybeep();
      else if(num==1){
         istr[0]=istr[0].substr(s.size());
         for(int i=0;i<istr[0].size();i++){
	    insertChar(istr[0][i],1);
         }
         insertChar(' ',1);
      }
      else{
...
CmdParser::getCmd(string cmd)
{
   ...
   CmdMap::iterator it;
   for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
      if((selfcompare(it->first,cmd,true))==true){
         string remain=cmd.substr((it->first).size());
   	  if((selfcompare((it->second)->getOptCmd(),remain,false))==true)
            return it->second;
       }
   }
   return e;
